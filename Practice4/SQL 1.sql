...
(5, 4, '2024-04-01', 'fer_2003', 'Today is sunny');
...
JOIN following F ON T.user_id = F.user_id
WHERE F.user_id_follower = 1;
#SELECT tweet_id,timestamp,text,usr FROM tweets AS T JOIN following AS F ON T.user_id = F.user_id_follower WHERE T.user_id = 1 GROUP BY T.tweet_id ORDER BY timestamp DESC LIMIT 0,8 ;